...
@section('header')
    ...
    <style>
        ...
        .profile-card .ad-name a:hover { color: #0d6efd; }
        ...
        .badge-paid { background:#e7f7ef; color:#13795b; }
        .badge-pending { background:#fff5e6; color:#b77400; }
        .badge-expired { background:#ffe9ea; color:#b4232a; }
        .badge-free { background:#eef1f5; color:#334155; }
        ...
        .btn-promote:hover { filter: brightness(.98); color:#111; }
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="ad-details">
                                        <p class="text-end ad-name mb-2">
                                            <a href="{{ route('the_posts.show', $post->id) }}">{{ Str::limit($post->title, 60) }}</a>
                                        </p>
                                        ...
                                        <div class="mb-2 text-start">
                                            @if($post->is_paid && $isActive)
                                                <span class="badge-status badge-paid">
                                                    <i class="fa fa-star"></i> إعلان مميز
                                                </span>
                                                @if($post->ends_at)
                                                    <small class="text-muted d-block mt-1">ينتهي: {{ $post->ends_at->diffForHumans() }}</small>
                                                @endif
                                            @elseif($post->status === 'pending_payment')
                                                <span class="badge-status badge-pending">
                                                    <i class="fa fa-clock-o"></i> في انتظار الدفع
                                                </span>
                                            @elseif($post->status === 'expired' || ($post->ends_at && $post->ends_at->lt(now())))
                                                <span class="badge-status badge-expired">
                                                    <i class="fa fa-times-circle"></i> منتهي
                                                </span>
                                            @else
                                                <span class="badge-status badge-free">
                                                    <i class="fa fa-circle-o"></i> إعلان مجاني
                                                </span>
                                            @endif
                                        </div>
                                        ...
                                        <div class="action-bar mt-3">
                                            @if($canPromote)
                                                <a href="{{ route('ads.promote', $post->id) }}"
                                            ...
                                            <form action="{{ route('the_posts.destroy', $post->id) }}"
                    ...
                    <div class="modal-body p-4">
                        <form method="POST" class="row g-3" action="{{ route('update_suggestions') }}">
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" class="form-control" value="{{ auth()->user()->city }}" id="city" name="city" autocomplete="city" required>
                            </div>
                            <div class="col-md-12">
                                <label for="birth_date" class="form-label"> تاريخ الميلاد </label>
                                <input type="date" class="form-control" value="{{ auth()->user()->birth_date }}" max="<?= date('Y-m-d'); ?>" id="birth_date" name="birth_date"
                                       autocomplete="birth_date" required>
                            </div>
                            <div class="col-md-12">
                                <label for="mobile" class="form-label"> الجوال </label>
                                <input type="text" class="form-control" value="{{ auth()->user()->mobile }}" id="mobile" name="mobile" maxlength="15" autocomplete="mobile" required>
                            </div>
                            <div class="col-md-12">
                                <label for="max_budget" class="form-label"> الميزانية القصوى المتاحة للدخول فى الفرص التجارية </label>
                                <input type="number" class="form-control" value="{{ auth()->user()->max_budget }}" id="max_budget" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==11) return false;" name="max_budget"
